...
})({"9IBXk":[function(require,module,exports) {
var global = arguments[3];
var HMR_HOST = null;
var HMR_PORT = null;
var HMR_SECURE = false;
var HMR_ENV_HASH = "d6ea1d42532a7575";
module.bundle.HMR_BUNDLE_ID = "5c2c2fd772bab410";
"use strict";
...
},{}],"5dahQ":[function(require,module,exports) {
...
const autoWrap = ()=>{
    ...
    return (0, _config389B86FfJs.g)().sequence.wrap;
};
...
const parseMessage = function(str) {
    ...
    (0, _config389B86FfJs.l).debug("parseMessage:", message);
    return message;
};
const parseBoxData = function(str) {
    ...
    const boxData = {
        color,
        text: title !== void 0 ? (0, _config389B86FfJs.b)(title.replace(/^:?(?:no)?wrap:/, ""), (0, _config389B86FfJs.g)()) : void 0,
...
const addLinks = function(actorId, text) {
    ...
    try {
        let sanitizedText = (0, _config389B86FfJs.b)(text.text, (0, _config389B86FfJs.g)());
        sanitizedText = sanitizedText.replace(/&amp;/g, "&");
        sanitizedText = sanitizedText.replace(/&equals;/g, "=");
        const links = JSON.parse(sanitizedText);
        insertLinks(actor, links);
    } catch (e) {
        (0, _config389B86FfJs.l).error("error while parsing actor link text", e);
    }
};
...
const addProperties = function(actorId, text) {
    ...
    try {
        ...
        (0, _config389B86FfJs.l).error("error while parsing actor properties text", e);
    }
};
...
const addDetails = function(actorId, text) {
    ...
    try {
        ...
        (0, _config389B86FfJs.l).error("error while parsing actor details text", e);
    }
};
...
const apply = function(param) {
    if (Array.isArray(param)) param.forEach(function(item) {
        ...
        case "altEnd":
            ...
            break;
        case "setAccTitle":
            (0, _commonDb2Ace122BJs.s)(param.text);
            break;
        case "parStart":
...
const db = {
    ...
    parseDirective,
    getConfig: ()=>(0, _config389B86FfJs.g)().sequence,
    clear,
    parseMessage,
    parseBoxData,
    LINETYPE,
...
const drawText = function(elem, textData) {
    ...
    const lines = textData.text.split((0, _config389B86FfJs.d).lineBreakRegex);
    const [_textFontSize, _textFontSizePx] = (0, _utilsD5Eeff82Js.t)(textData.fontSize);
    let textElems = [];
    let dy = 0;
...
const _drawTextCandidateFunc = function() {
    ...
    function byTspan(content, g, x, y, width, height, textAttrs, conf2) {
        ...
        const [_actorFontSize, _actorFontSizePx] = (0, _utilsD5Eeff82Js.t)(actorFontSize);
        const lines = content.split((0, _config389B86FfJs.d).lineBreakRegex);
...
function boundMessage(_diagram, msgModel) {
    ...
    const lines = (0, _config389B86FfJs.d).splitBreaks(message).length;
    const textDims = (0, _utilsD5Eeff82Js.u).calculateTextDimensions(message, messageFont(conf));
...
const drawMessage = function(diagram2, msgModel, lineStartY, diagObj) {
    ...
    let line;
    if (startx === stopx) {
        if (conf.rightAngles) line = diagram2.append("path").attr("d", `M  ${startx},${lineStartY} H ${startx + Math.max(conf.width / 2, textWidth / 2)} V ${lineStartY + 25} H ${startx}`);
...
const setConf = function(cnf) {
    (0, _config389B86FfJs.c)(conf, cnf);
...
function adjustLoopHeightForWrap(loopWidths, msg, preMargin, postMargin, addLoopFn) {
    ...
    if (msg.id && msg.message && loopWidths[msg.id]) {
        ...
        msg.message = (0, _utilsD5Eeff82Js.u).wrapLabel(`[${msg.message}]`, loopWidth - 2 * conf.wrapPadding, textConf);
        msg.width = loopWidth;
        msg.wrap = true;
        const textDims = (0, _utilsD5Eeff82Js.u).calculateTextDimensions(msg.message, textConf);
        const totalOffset = Math.max(textDims.height, conf.labelBoxHeight);
        heightAdjust = postMargin + totalOffset;
        (0, _config389B86FfJs.l).debug(`${totalOffset} - ${msg.message}`);
    }
    addLoopFn(msg);
    bounds.bumpVerticalPos(heightAdjust);
}
const draw = function(_text, id, _version, diagObj) {
    const { securityLevel , sequence  } = (0, _config389B86FfJs.g)();
    conf = sequence;
    diagObj.db.clear();
    ...
    bounds.init();
    (0, _config389B86FfJs.l).debug(diagObj.db);
        ...
        switch(msg.type){
            ...
            default:
                try {
                    ...
                    (0, _config389B86FfJs.l).error("error while drawing message", e);
                }
        }
        if ([
    ...
    (0, _config389B86FfJs.l).debug("For line height fix Querying: #" + id + " .actor-line");
    ...
    (0, _config389B86FfJs.l).debug(`models:`, bounds.models);
};
function getMaxMessageWidthPerActor(actors2, messages2, diagObj) {
    ...
    messages2.forEach(function(msg) {
        if (actors2[msg.to] && actors2[msg.from]) {
            ...
            const wrappedMessage = msg.wrap ? (0, _utilsD5Eeff82Js.u).wrapLabel(msg.message, conf.width - 2 * conf.wrapPadding, textFont) : msg.message;
            const messageDimensions = (0, _utilsD5Eeff82Js.u).calculateTextDimensions(wrappedMessage, textFont);
            const messageWidth = messageDimensions.width + 2 * conf.wrapPadding;
            if (isMessage && msg.from === actor.nextActor) maxMessageWidthPerActor[msg.to] = Math.max(maxMessageWidthPerActor[msg.to] || 0, messageWidth);
            else if (isMessage && msg.from === actor.prevActor) maxMessageWidthPerActor[msg.from] = Math.max(maxMessageWidthPerActor[msg.from] || 0, messageWidth);
            ...
            } else if (msg.placement === diagObj.db.PLACEMENT.RIGHTOF) maxMessageWidthPerActor[msg.from] = Math.max(maxMessageWidthPerActor[msg.from] || 0, messageWidth);
            else if (msg.placement === diagObj.db.PLACEMENT.LEFTOF) maxMessageWidthPerActor[actor.prevActor] = Math.max(maxMessageWidthPerActor[actor.prevActor] || 0, messageWidth);
            else if (msg.placement === diagObj.db.PLACEMENT.OVER) {
                if (actor.prevActor) maxMessageWidthPerActor[actor.prevActor] = Math.max(maxMessageWidthPerActor[actor.prevActor] || 0, messageWidth / 2);
                if (actor.nextActor) maxMessageWidthPerActor[msg.from] = Math.max(maxMessageWidthPerActor[msg.from] || 0, messageWidth / 2);
            }
        }
    });
    (0, _config389B86FfJs.l).debug("maxMessageWidthPerActor:", maxMessageWidthPerActor);
...
const getRequiredPopupWidth = function(actor) {
    ...
    for(const key in actor.links){
        const labelDimensions = (0, _utilsD5Eeff82Js.u).calculateTextDimensions(key, textFont);
...
function calculateActorMargins(actors2, actorToMessageWidth, boxes2) {
    ...
    Object.keys(actors2).forEach((prop)=>{
        ...
        if (actor.wrap) actor.description = (0, _utilsD5Eeff82Js.u).wrapLabel(actor.description, conf.width - 2 * conf.wrapPadding, actorFont(conf));
        const actDims = (0, _utilsD5Eeff82Js.u).calculateTextDimensions(actor.description, actorFont(conf));
    ...
    boxes2.forEach((box)=>{
        ...
        if (box.wrap) box.name = (0, _utilsD5Eeff82Js.u).wrapLabel(box.name, totalWidth - 2 * conf.wrapPadding, textFont);
        const boxMsgDimensions = (0, _utilsD5Eeff82Js.u).calculateTextDimensions(box.name, textFont);
...
const buildNoteModel = function(msg, actors2, diagObj) {
    ...
    let textDimensions = (0, _utilsD5Eeff82Js.u).calculateTextDimensions(shouldWrap ? (0, _utilsD5Eeff82Js.u).wrapLabel(msg.message, conf.width, noteFont(conf)) : msg.message, noteFont(conf));
    const noteModel = {
    ...
    if (msg.placement === diagObj.db.PLACEMENT.RIGHTOF) {
        ...
        textDimensions = (0, _utilsD5Eeff82Js.u).calculateTextDimensions(shouldWrap ? (0, _utilsD5Eeff82Js.u).wrapLabel(msg.message, Math.max(conf.width, actors2[msg.from].width), noteFont(conf)) : msg.message, noteFont(conf));
    ...
    if (shouldWrap) noteModel.message = (0, _utilsD5Eeff82Js.u).wrapLabel(msg.message, noteModel.width - 2 * conf.wrapPadding, noteFont(conf));
    (0, _config389B86FfJs.l).debug(`NM:[${noteModel.startx},${noteModel.stopx},${noteModel.starty},${noteModel.stopy}:${noteModel.width},${noteModel.height}=${msg.message}]`);
    return noteModel;
};
const buildMessageModel = function(msg, actors2, diagObj) {
    ...
    if (msg.wrap && msg.message) msg.message = (0, _utilsD5Eeff82Js.u).wrapLabel(msg.message, Math.max(boundedWidth + 2 * conf.wrapPadding, conf.width), messageFont(conf));
    const msgDims = (0, _utilsD5Eeff82Js.u).calculateTextDimensions(msg.message, messageFont(conf));
    return {
...
const calculateLoopBounds = function(messages2, actors2, _maxWidthPerActor, diagObj) {
    ...
    messages2.forEach(function(msg) {
        msg.id = (0, _utilsD5Eeff82Js.u).random({
            length: 10
        });
        switch(msg.type){
            case diagObj.db.LINETYPE.LOOP_START:
    ...
    bounds.activations = [];
    (0, _config389B86FfJs.l).debug("Loop type widths:", loops);
    return loops;
};
const renderer = {
    bounds,
...
},{"./mermaidAPI-0716c7c2.js":"bdkW7","./config-389b86ff.js":"hnpRM","./commonDb-2ace122b.js":"kBdnX","d3":"17XFv","./utils-d5eeff82.js":"2zv2d","@braintree/sanitize-url":"hrxAd","./setupGraphViewbox-e35e4124.js":"jv1OR","stylis":"bMCXt","./errorRenderer-d05351b9.js":"d3Vzc","dompurify":"9Kzno","lodash-es/isEmpty.js":"e1Kmi","dayjs":"NJZFB","khroma":"lMw0x","lodash-es/memoize.js":"5zdei","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"kBdnX":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
...
},{"d3":"17XFv","./config-389b86ff.js":"hnpRM","./utils-d5eeff82.js":"2zv2d","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}]},["9IBXk"], null, "parcelRequire54bf")

//# sourceMappingURL=sequenceDiagram-2c95880e.72bab410.js.map
