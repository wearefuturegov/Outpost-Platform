...
})({"38dNz":[function(require,module,exports) {
var global = arguments[3];
var HMR_HOST = null;
var HMR_PORT = null;
var HMR_SECURE = false;
var HMR_ENV_HASH = "d6ea1d42532a7575";
module.bundle.HMR_BUNDLE_ID = "c1b62c5ce4ba1e56";
"use strict";
...
},{}],"b5K9s":[function(require,module,exports) {
...
var _isoWeekJs = require("dayjs/plugin/isoWeek.js");
var _isoWeekJsDefault = parcelHelpers.interopDefault(_isoWeekJs);
var _customParseFormatJs = require("dayjs/plugin/customParseFormat.js");
var _customParseFormatJsDefault = parcelHelpers.interopDefault(_customParseFormatJs);
var _advancedFormatJs = require("dayjs/plugin/advancedFormat.js");
var _advancedFormatJsDefault = parcelHelpers.interopDefault(_advancedFormatJs);
...
(0, _dayjsDefault.default).extend((0, _isoWeekJsDefault.default));
(0, _dayjsDefault.default).extend((0, _customParseFormatJsDefault.default));
(0, _dayjsDefault.default).extend((0, _advancedFormatJsDefault.default));
let dateFormat = "";
let axisFormat = "";
...
const checkTaskDates = function(task, dateFormat2, excludes2, includes2) {
    ...
    let startTime;
    if (task.startTime instanceof Date) startTime = (0, _dayjsDefault.default)(task.startTime);
    else startTime = (0, _dayjsDefault.default)(task.startTime, dateFormat2, true);
    startTime = startTime.add(1, "d");
    let originalEndTime;
    if (task.endTime instanceof Date) originalEndTime = (0, _dayjsDefault.default)(task.endTime);
    else originalEndTime = (0, _dayjsDefault.default)(task.endTime, dateFormat2, true);
...
const getStartDate = function(prevTime, dateFormat2, str) {
    ...
    let mDate = (0, _dayjsDefault.default)(str, dateFormat2.trim(), true);
    if (mDate.isValid()) return mDate.toDate();
    else {
        (0, _config389B86FfJs.l).debug("Invalid date:" + str);
        (0, _config389B86FfJs.l).debug("With date format:" + dateFormat2.trim());
        const d = new Date(str);
        if (d === void 0 || isNaN(d.getTime())) throw new Error("Invalid date:" + str);
        return d;
    }
};
...
const getEndDate = function(prevTime, dateFormat2, str, inclusive = false) {
    ...
    let endTime = (0, _dayjsDefault.default)(prevTime);
...
const compileData = function(prevTask, dataStr) {
    ...
    if (endTimeData) {
        ...
        task.manualEndTime = (0, _dayjsDefault.default)(endTimeData, "YYYY-MM-DD", true).isValid();
    ...
    const compileTask = function(pos) {
        ...
        if (rawTasks[pos].startTime) {
            ...
            if (rawTasks[pos].endTime) {
                ...
                rawTasks[pos].manualEndTime = (0, _dayjsDefault.default)(rawTasks[pos].raw.endTime.data, "YYYY-MM-DD", true).isValid();
...
const setLink = function(ids, _linkStr) {
    ...
    if ((0, _config389B86FfJs.g)().securityLevel !== "loose") linkStr = (0, _sanitizeUrl.sanitizeUrl)(_linkStr);
...
const setClickFun = function(id, functionName, functionArgs) {
    if ((0, _config389B86FfJs.g)().securityLevel !== "loose") return;
    ...
    let rawTask = findTaskById(id);
    if (rawTask !== void 0) pushFun(id, ()=>{
        (0, _utilsD5Eeff82Js.u).runFunc(functionName, ...argList);
    });
};
...
const ganttDb = {
    parseDirective,
    getConfig: ()=>(0, _config389B86FfJs.g)().gantt,
    clear,
    setDateFormat,
    getDateFormat,
...
const setConf = function() {
    (0, _config389B86FfJs.l).debug("Something is calling, setConf, remove the call");
};
let w;
const draw = function(text, id, version, diagObj) {
    const conf = (0, _config389B86FfJs.g)().gantt;
    ...
    const timeScale = (0, _d3.scaleTime)().domain([
        (0, _d3.min)(taskArray, function(d) {
            return d.startTime;
        }),
        (0, _d3.max)(taskArray, function(d) {
            return d.endTime;
        })
    ]).rangeRound([
        0,
    ...
    (0, _setupGraphViewboxE35E4124Js.c)(svg, h, w, conf.useMaxWidth);
    ...
    function makeGant(tasks2, pageWidth, pageHeight) {
        ...
        const colorScale = (0, _d3.scaleLinear)().domain([
            0,
            categories.length
        ]).range([
            "#00B9FA",
            "#F95002"
        ]).interpolate((0, _d3.interpolateHcl));
    ...
    function drawRects(theArray, theGap, theTopPad, theSidePad, theBarHeight, theColorScale, w2) {
        ...
        const securityLevel2 = (0, _config389B86FfJs.g)().securityLevel;
        if (securityLevel2 === "sandbox") {
            let sandboxElement2;
            sandboxElement2 = (0, _d3.select)("#i" + id);
    ...
    function drawExcludeDays(theGap, theTopPad, theSidePad, w2, h2, tasks2, excludes2, includes2) {
        const minTime = tasks2.reduce((min2, { startTime  })=>min2 ? Math.min(min2, startTime) : startTime, 0);
        const maxTime = tasks2.reduce((max2, { endTime  })=>max2 ? Math.max(max2, endTime) : endTime, 0);
        const dateFormat2 = diagObj.db.getDateFormat();
        if (!minTime || !maxTime) return;
        const excludeRanges = [];
        let range = null;
        let d = (0, _dayjsDefault.default)(minTime);
    ...
    function makeGrid(theSidePad, theTopPad, w2, h2) {
        let bottomXAxis = (0, _d3.axisBottom)(timeScale).tickSize(-h2 + theTopPad + conf.gridLineStartPadding).tickFormat((0, _d3.timeFormat)(diagObj.db.getAxisFormat() || conf.axisFormat || "%Y-%m-%d"));
        ...
        if (resultTickInterval !== null) {
            ...
            switch(interval){
                case "minute":
                    bottomXAxis.ticks((0, _d3.timeMinute).every(every));
                    break;
                case "hour":
                    bottomXAxis.ticks((0, _d3.timeHour).every(every));
                    break;
                case "day":
                    bottomXAxis.ticks((0, _d3.timeDay).every(every));
                    break;
                case "week":
                    bottomXAxis.ticks((0, _d3.timeWeek).every(every));
                    break;
                case "month":
                    bottomXAxis.ticks((0, _d3.timeMonth).every(every));
                    break;
            }
        }
        svg.append("g").attr("class", "grid").attr("transform", "translate(" + theSidePad + ", " + (h2 - 50) + ")").call(bottomXAxis).selectAll("text").style("text-anchor", "middle").attr("fill", "#000").attr("stroke", "none").attr("font-size", 10).attr("dy", "1em");
        if (diagObj.db.topAxisEnabled() || conf.topAxis) {
            let topXAxis = (0, _d3.axisTop)(timeScale).tickSize(-h2 + theTopPad + conf.gridLineStartPadding).tickFormat((0, _d3.timeFormat)(diagObj.db.getAxisFormat() || conf.axisFormat || "%Y-%m-%d"));
            if (resultTickInterval !== null) {
                const every = resultTickInterval[1];
                const interval = resultTickInterval[2];
                switch(interval){
                    case "minute":
                        topXAxis.ticks((0, _d3.timeMinute).every(every));
                        break;
                    case "hour":
                        topXAxis.ticks((0, _d3.timeHour).every(every));
                        break;
                    case "day":
                        topXAxis.ticks((0, _d3.timeDay).every(every));
                        break;
                    case "week":
                        topXAxis.ticks((0, _d3.timeWeek).every(every));
                        break;
                    case "month":
                        topXAxis.ticks((0, _d3.timeMonth).every(every));
                        break;
                }
            }
    ...
    function vertLabels(theGap, theTopPad) {
        const numOccurances = [];
        let prevGap = 0;
        for (const [i, category] of categories.entries())numOccurances[i] = [
            category,
            getCount(category, catsUnfiltered)
        ];
        svg.append("g").selectAll("text").data(numOccurances).enter().append(function(d) {
            const rows = d[0].split((0, _config389B86FfJs.d).lineBreakRegex);
...
},{"@braintree/sanitize-url":"hrxAd","dayjs":"NJZFB","dayjs/plugin/isoWeek.js":"cgWk7","dayjs/plugin/customParseFormat.js":"dmrpe","dayjs/plugin/advancedFormat.js":"aH65B","./config-389b86ff.js":"hnpRM","./utils-d5eeff82.js":"2zv2d","./mermaidAPI-0716c7c2.js":"bdkW7","./commonDb-2ace122b.js":"kBdnX","d3":"17XFv","./setupGraphViewbox-e35e4124.js":"jv1OR","dompurify":"9Kzno","khroma":"lMw0x","lodash-es/memoize.js":"5zdei","stylis":"bMCXt","./errorRenderer-d05351b9.js":"d3Vzc","lodash-es/isEmpty.js":"e1Kmi","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"cgWk7":[function(require,module,exports) {
!function(e, t) {
    module.exports = t();
}(this, function() {
    "use strict";
...
},{}],"kBdnX":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
...
},{"d3":"17XFv","./config-389b86ff.js":"hnpRM","./utils-d5eeff82.js":"2zv2d","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}]},["38dNz"], null, "parcelRequire54bf")

//# sourceMappingURL=ganttDiagram-41439120.e4ba1e56.js.map
